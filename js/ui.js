export class UI {
  ...
  updateTurn(player, round, total = 5, isSudden = false) {
    ...
    if (isSudden) {
      document.getElementById('turn-round').textContent = `MORTE SÚBITA · RODADA ${round}`;
    } else {
      document.getElementById('turn-round').textContent = `COBRANÇA ${round}/${total}`;
    }
  }
  ...
  showResult(type) {
    ...
    if (type === 'goal') {
      el.innerHTML = '<span class="rm-icon">⚽</span><span class="rm-text">GOOOOOL!</span><span class="rm-sub">QUE GOLAÇO!</span>';
    } else if (type === 'save') {
      el.innerHTML = '<span class="rm-icon">🧤</span><span class="rm-text">DEFENDEU!</span><span class="rm-sub">QUE DEFESA!</span>';
    } else {
      el.innerHTML = '<span class="rm-icon">❌</span><span class="rm-text">FORA!</span><span class="rm-sub">ERROU O ALVO</span>';
    }
  ...
  spawnConfetti(count = 40) {
    ...
    const colors = ['#ffd700', '#ff3366', '#00ccff', '#ff6b00', '#ffffff', '#a855f7', '#ff8fab'];
    for (let i = 0; i < count; i++) {
  ...
  buildCrowd() {
    const colors = [
      ...
      '#8e44ad', '#9b59b6', '#f39c12', '#e67e22', '#1abc9c', '#34495e',
      '#ffffff', '#f1c40f', '#d35400', '#2c3e50', '#16a085'
    ];
  ...
  renderHistory(list) {
    ...
    if (!list.length) {
      el.innerHTML = '<div class="history-empty">Nenhuma partida registrada ainda.</div>';
      return;
    }
